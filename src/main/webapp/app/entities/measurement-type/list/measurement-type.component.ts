...
export class MeasurementTypeComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.measurementTypeService
      ...
      .subscribe(
        (res: HttpResponse<IMeasurementType[]>) => {
  ...
  trackId(index: number, item: IMeasurementType): string {
    return item.id!;
  }

  delete(measurementType: IMeasurementType): void {
    const modalRef = this.modalService.open(MeasurementTypeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.measurementType = measurementType;
  ...
  protected onSuccess(data: IMeasurementType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/measurement-type'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
