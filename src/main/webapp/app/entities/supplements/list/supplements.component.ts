...
export class SupplementsComponent implements OnInit {
  ...
  trackId(index: number, item: ISupplements): string {
    return item.id!;
  }
  ...
  delete(supplements: ISupplements): void {
    const modalRef = this.modalService.open(SupplementsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.supplements = supplements;
  ...
  protected onSuccess(data: ISupplements[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
