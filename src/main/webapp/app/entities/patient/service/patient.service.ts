...
export class PatientService {
  ...
  protected convertDateFromClient(patient: IPatient): IPatient {
    return Object.assign({}, patient, {
      admissionDate: patient.admissionDate?.isValid() ? patient.admissionDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((patient: IPatient) => {
        patient.admissionDate = patient.admissionDate ? dayjs(patient.admissionDate) : undefined;
      });
    }
    return res;
  }
